...
- Circuit propagation can summarize discarded coefficients either with an :math:`\ell_1`-style sum of magnitudes or with an :math:`\ell_2`-style aggregate, selected by the ``truncation_norm`` argument of :func:`~pauli_prop.propagation.propagate_through_circuit` and :func:`~pauli_prop.propagation.propagate_through_rotation_gates`.
...
``propagate_through_rotation_gates``: As the Pauli operator is propagated in the Pauli basis under the action of a sequence of :math:`N` Pauli rotation gates of an :math:`M`-qubit circuit, the number of terms will grow as :math:`\mathcal{O}(2^N)` towards a maximum of :math:`4^M` unique Pauli components. To control memory usage, the operator is truncated after application of each gate, which introduces some error proportional to the magnitudes of the truncated terms' coefficients. The second returned value summarizes discarded magnitudes across the evolution: by default it is an :math:`\ell_1`-style sum; passing ``truncation_norm="l2"`` selects the Euclidean-style aggregate :math:`\sqrt{\sum_i c_i^2}` built from the same discards (truncation itself is unchanged). The memory requirements are linear in the size of the evolved operator and runtime scales linearly in both the operator size and the number of gates.
